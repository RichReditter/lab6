...
let day = 1;
let experiment;
for (experiment = 1; experiment < 100; experiment++) {
  while (progress < 100) {
    for (let i = 0; i < types.length; i++) {
      if (progress == 0) {
        if (levels.highLvl == i) {
          ...
          let amountOfLowLvl = Math.floor(
            types[levels.lowLvl] * amountOfWorkers
          );
...
days.forEach((day) => {
  daysNumber = daysNumber + day;
});
let middleValue = daysNumber / days.length - 1;
console.log(
  `Среднее значение дней, за которое задача была выполнена, при 100 экспериментах: \n ${middleValue}`
);
